...
async function sendMonitorAlert(coins) {
  // Menor LSR com RSI 15m/1h baixo e OI subindo no 5m
  ...
  // Maior LSR com RSI 15m/1h alto e OI caindo no 5m
  ...
  logger.info(`Menores LSR com RSI baixo e OI subindo: ${topLowLsrLowRsi.length}, Maiores LSR com RSI alto e OI caindo: ${topHighLsrHighRsi.length}`);

  // Alerta para menores LSR com RSI baixo e OI subindo
  if (topLowLsrLowRsi.length > 0) {
    let lowLsrLowRsiAlertText = `🟢*Menor LSR com RSI Baixo 15m/1h e OI Subindo (5m) 🚀*\n\n`;
    ...
    logger.info(`Tamanho da mensagem de Menores LSR com RSI baixo e OI subindo: ${lowLsrLowRsiAlertText.length} caracteres`);
    await sendTelegramMessage(lowLsrLowRsiAlertText);
    logger.info('Alerta de moedas com Menores LSR, RSI baixo e OI subindo enviado com sucesso');
  }

  // Alerta para maiores LSR com RSI alto e OI caindo
  if (topHighLsrHighRsi.length > 0) {
    let highLsrHighRsiAlertText = `🔴*Maior LSR com RSI Alto 15m/1h e OI Caindo (5m) 📉*\n\n`;
    ...
    logger.info(`Tamanho da mensagem de Maiores LSR com RSI alto e OI caindo: ${highLsrHighRsiAlertText.length} caracteres`);
    await sendTelegramMessage(highLsrHighRsiAlertText);
    logger.info('Alerta de moedas com Maiores LSR, RSI alto e OI caindo enviado com sucesso');
  }
