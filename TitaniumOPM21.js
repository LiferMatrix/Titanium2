...
async function sendMonitorAlert(coins) {
  ...
  if (starCoins.length > 0) {
    let starAlertText = `🟢*Compra-💥Operação Filtrada💥 *\n\n`;
  ...
  if (skullCoins.length > 0) {
    let skullAlertText = `🔴*Correção-💥Operação Filtrada💥 *\n\n`;
...
async function main() {
  ...
  try {
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium Filtrado Ativado...!'));
    await checkCoins();
